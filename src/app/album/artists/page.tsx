...
async function fetchArtists(query: string, limit = 12): Promise<ArtistPreview[]> {
  try {
    ...
    console.error("Failed to fetch artists", query, error);
    return [];
  }
}
        ...
        <div className="relative grid gap-6 lg:grid-cols-[2fr,1fr] lg:items-center">
          <div className="space-y-5">
            ...
            <p className="text-muted-foreground text-lg max-w-2xl">
              Explore charting singers, timeless legends, and indie discoveries. Tap an artist to
              open their page and start a personalized queue instantly.
            </p>
            ...
            <div className="flex flex-wrap gap-3 text-muted-foreground text-sm">
              <span className="rounded-full border border-border px-3 py-1 bg-secondary/70">
                Verified & trending
              </span>
              <span className="rounded-full border border-border px-3 py-1 bg-secondary/70">
                One-tap artist pages
              </span>
              <span className="rounded-full border border-border px-3 py-1 bg-secondary/70">
                Likes sync everywhere
              </span>
            </div>
          </div>
            ...
            {heroPicks.map((artist) => (
              ...
              >
                ...
                <p className="font-semibold text-lg line-clamp-1">
                  {artist.name.split("(")[0].replace("&#039;", "'")}
                </p>
                <p className="text-xs text-muted-foreground mt-1 line-clamp-2">
                  {artist.blurb || "Tap to open the artist page"}
                </p>
              </div>
            ))}
          </div>
        ...
        <section key={section.key} className="space-y-4">
          ...
          {section.artists.length === 0 ? (
            <p className="text-muted-foreground">
              No artists found right now. Try the search above.
            </p>
          ) : (
